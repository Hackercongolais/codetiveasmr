...
class Carousel {
  ...
  slide(forward) {
        ...
        setTimeout(() => {
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              ...
              this.elements.items.center.classList.remove(
    ...
    setTimeout(() => {
      this.elements.items.left = this.target.querySelector(
        ".carousel__item--left:not(.carousel__item--out)"
      );
      this.elements.items.center = this.target.querySelector(
        ".carousel__item--center"
      );
      this.elements.items.right = this.target.querySelector(
        ".carousel__item--right:not(.carousel__item--out)"
      );
      this.elements.items.out = this.target.querySelector(
        ".carousel__item--out"
      );
    ...
    if (!this.animating) {
      ...
      switch (forward) {
        case true:
          ...
          newItem =
            this.active + 1 <= this.items.length - 1 ? this.active + 1 : 0;
          break;
        ...
        case false:
          ...
          newItem =
            this.active - 1 >= 0 ? this.active - 1 : this.items.length - 1;
          break;
      }
  ...
  eventListeners() {
    this.elements.arrows.left.addEventListener(
      "click",
      this.updateValues.bind(this, false)
    );
    this.elements.arrows.right.addEventListener(
      "click",
      this.updateValues.bind(this, true)
    );
  }
  ...
  populate() {
    ...
    this.elements.items.left.style.backgroundImage = `url('${
      this.items[this.items.length - 1].image
    }')`;
